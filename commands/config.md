...
2. Otherwise, use **AskUserQuestion**: "Which Claude model should Nexus use?" (header: "Model", options: "opus (default)", "sonnet", "haiku", "glm")
3. Read `.claude/claudeclaw/settings.json`.
...
1. If token is in `$ARGUMENTS`, use it directly.
2. Otherwise, use **AskUserQuestion**: "What API token should Nexus use for glm?" (header: "API token", options: let user type via Other)
3. Read `.claude/claudeclaw/settings.json`.
...
2. Otherwise, use **AskUserQuestion**: "Which fallback model should Nexus use?" (header: "Fallback model", options: "glm (Recommended)", "sonnet", "haiku")
3. Read `.claude/claudeclaw/settings.json`.
...
1. If token is in `$ARGUMENTS`, use it directly.
2. Otherwise, use **AskUserQuestion**: "What API token should Nexus use for fallback model?" (header: "Fallback API token", options: let user type via Other)
3. Read `.claude/claudeclaw/settings.json`.
...
2. Otherwise, use **AskUserQuestion**: "What timezone should Nexus use?" (header: "Timezone", options: "UTC (Recommended)", "America/New_York", "Europe/London")
3. Read `.claude/claudeclaw/settings.json`.
