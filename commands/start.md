...
1. **Block home-directory starts (CRITICAL, BLOCKER)**:
   ...
   - Tell the user exactly:
     - "CRITICAL BLOCKER: For security reasons, close this session and start a new one from the folder you want to initialize Nexus in."
...
5. **Ask setup questions**:
   ...
   - **Model** (always ask if `model` is empty/unset): "Which Claude model should Nexus use?" (header: "Model", options: "opus (default)", "sonnet", "haiku", "glm")
...
6. **Launch/start action**:
   ...
   ```
   Use the description "Starting Nexus server" for this command.
