...
import { initConfig, loadSettings, reloadSettings, resolvePrompt, type HeartbeatConfig, type Settings, type SlackConfig } from "../config";
import { getDayAndMinuteAtOffset } from "../timezone";
import { startWebUi, type WebServerHandle } from "../web";
import type { Job } from "../jobs";
import { readPendingTasks, markInProgress, completeTask, failTask } from "../tasks";
import { notifyTaskResult } from "../notify";
import { loadNexus, resolveCurrentEntity } from "../nexus";
...
var BR = DIM + "\\u256f" + R;
var H = DIM + "\\u2500" + R;
var HEADER = TL + H.repeat(11) + " Nexus " + H.repeat(12) + TR;
...
export async function start(args: string[] = []) {
  ...
  if (hasPromptFlag && !payload) {
    console.error("Usage: nexus start --prompt <prompt> [--trigger] [--telegram] [--discord] [--debug] [--web] [--web-port <port>] [--replace-existing]");
    process.exit(1);
  }
  if (!hasPromptFlag && payload) {
  ...
  if (hasPromptFlag && !hasTriggerFlag) {
    ...
    if (existingPid) {
      ...
      console.error("Use `nexus send <message> [--telegram] [--discord]` while daemon is running.");
      process.exit(1);
    }
  ...
  console.log("Nexus daemon started");
  ...
  async function initSlack(cfg: SlackConfig) {
    if (cfg.token && cfg.channelId && cfg.token !== slackToken) {
      const { startPolling } = await import("./slack");
      startPolling(debugFlag);
      slackToken = cfg.token;
      console.log(`[${ts()}] Slack: enabled (channel ${cfg.channelId})`);
    } else if (!cfg.token && slackToken) {
      slackToken = "";
      console.log(`[${ts()}] Slack: disabled`);
    }
  }

  await initSlack(currentSettings.slack);
  ...
  // --- Auto-start sibling entities ---
  try {
    const nexus = await loadNexus();
    const currentEntity = resolveCurrentEntity(nexus);
    if (currentEntity === nexus.orchestrator) {
      for (const [name, entity] of Object.entries(nexus.entities)) {
        if (name === currentEntity) continue;
        if (!entity.autostart) continue;
        const pidFile = join(entity.path, ".claude/claudeclaw/daemon.pid");
        ...
        if (alreadyRunning) {
          console.log(`[${ts()}] Auto-start: ${name} already running`);
          continue;
        }
        const logFile = join(entity.path, ".claude/claudeclaw/logs/daemon.log");
        const proc = Bun.spawn(
          ["bun", "run", join(nexus.corePath, "src/index.ts"), "start"],
          { cwd: entity.path, stdin: "ignore", stdout: Bun.file(logFile), stderr: Bun.file(logFile) }
        );
        proc.unref();
        console.log(`[${ts()}] Auto-start: spawned ${name} (PID ${proc.pid})`);
      }
    }
  } catch (err) {
    console.error(`[${ts()}] Auto-start error:`, err);
  }
  ...
  async function processInboxTasks() {
    ...
    let pending;
    try {
      pending = await readPendingTasks(cwd);
    } catch {
      return;
    }
    for (const task of pending) {
      const inProgress = await markInProgress(cwd, task).catch(() => task);
      run(`task:${task.id.slice(-8)}`, inProgress.prompt)
        .then(async (result) => {
          if (result.exitCode === 0) {
            const done = await completeTask(cwd, inProgress, result.stdout);
            await notifyTaskResult(done);
          } else {
            const done = await failTask(cwd, inProgress, result.stderr || result.stdout);
            await notifyTaskResult(done);
          }
        })
        .catch(async (err) => {
          const done = await failTask(cwd, inProgress, String(err)).catch(() => inProgress);
          await notifyTaskResult({ ...done, status: "failed", error: String(err) });
        });
    }
  }
  ...
  setInterval(() => {
    processInboxTasks().catch((err) =>
      console.error(`[${ts()}] Task inbox error:`, err)
    );
