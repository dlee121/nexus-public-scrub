import { ensureProjectClaudeMd, run, runUserMessage, getLastRunEntities, compactCurrentSession } from "../runner";
...
let nexusEntities: Record<string, NexusEntityCfg> | null = null;

async function loadNexusEntities(): Promise<Record<string, NexusEntityCfg>> {
  if (nexusEntities) return nexusEntities;
  try {
    const nexusPath = join(import.meta.dir, "..", "..", "nexus.json");
    const data = await Bun.file(nexusPath).json() as { entities?: Record<string, NexusEntityCfg> };
    nexusEntities = data.entities ?? {};
  } catch {
    nexusEntities = {};
  }
  return nexusEntities;
}
...
async function readEntityModel(entityPath: string): Promise<string> {
  try {
    const settingsPath = join(entityPath, ".claude", "claudeclaw", "settings.json");
    const data = await Bun.file(settingsPath).json() as { model?: string };
    return shortModel(data.model ?? "");
  } catch {
    return "sonnet";
  }
}

let cachedPrimaryEntity: EntityInfo | null = null;

async function resolvePrimaryEntity(): Promise<EntityInfo> {
  if (cachedPrimaryEntity) return cachedPrimaryEntity;
  const entities = await loadNexusEntities();
  const cwd = process.cwd();
  for (const [name, cfg] of Object.entries(entities)) {
    if (cfg.path === cwd) {
      const model = await readEntityModel(cwd);
      cachedPrimaryEntity = { name, emoji: cfg.emoji ?? "⚡", model };
      return cachedPrimaryEntity;
    }
  }
  cachedPrimaryEntity = { name: "nexus", emoji: "⚡", model: shortModel(getSettings().model) };
  return cachedPrimaryEntity;
}

async function resolveSubEntityLabel(subagentType: string): Promise<string> {
  const entities = await loadNexusEntities();
  const cfg = entities[subagentType];
  if (cfg?.path) {
    const model = await readEntityModel(cfg.path);
    const emoji = cfg.emoji ? `${cfg.emoji} ` : "";
    return `${emoji}${subagentType} (${model})`;
  }
  return subagentType;
}
...
async function handleMessage(message: TelegramMessage): Promise<void> {
  ...
  try {
    ...
    if (result.exitCode !== 0) {
      ...
      const allEntities = await loadNexusEntities();
      const subNames = getLastRunEntities().filter((e) => e !== primary.name);
      const nexusSubs: string[] = [];
      const ccAgents: string[] = [];
      for (const name of subNames) {
        if (allEntities[name]) nexusSubs.push(await resolveSubEntityLabel(name));
        else ccAgents.push(name);
      }
      const entityParts = [`${primary.emoji} ${primary.name} (${primary.model})`, ...nexusSubs];
      const ccSuffix = ccAgents.length > 0 ? ` · cc: ${ccAgents.join(", ")}` : "";
      const footer = `\n— ${entityParts.join(" + ")} · ${elapsedStr}${ccSuffix}`;
      if (cleanedText || filePaths.length === 0) {
        await sendMessage(config.token, chatId, (cleanedText || "(empty response)") + footer, threadId);
      }
