...
/** Reset turnCount (and clear compactWarned) for a thread session after a
 *  successful compact. Without this, turnCount monotonically climbs past
 *  PROACTIVE_COMPACT_THRESHOLD and the threshold check in runner.ts fires
 *  on every subsequent turn forever. */
export async function resetThreadTurnCount(threadId: string): Promise<void> {
  const data = await loadSessions();
  const session = data.threads[threadId];
  if (!session) return;
  session.turnCount = 0;
