...
/** Reset turnCount (and clear compactWarned) after a successful compact.
 *  Without this, turnCount monotonically climbs past PROACTIVE_COMPACT_THRESHOLD
 *  and the threshold check in runner.ts fires on every subsequent turn forever,
 *  burning tokens compacting an already-compacted session. */
export async function resetTurnCount(): Promise<void> {
  const existing = await loadSession();
  if (!existing) return;
