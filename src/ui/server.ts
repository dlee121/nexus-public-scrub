...
import { getActiveRuns, getActiveRunOutput } from "../runner";
  ...
  const server = Bun.serve({
    ...
    fetch: async (req) => {
      ...
      if (url.pathname === "/api/runs/active") {
        return json(getActiveRuns());
      }

      if (url.pathname === "/api/runs/output") {
        const id = url.searchParams.get("id") ?? "";
        const output = getActiveRunOutput(id);
        if (output === null) return new Response("Not found", { status: 404 });
        return json({ output });
      }
