export const pageScript = String.raw`    const $ = (id) => document.getElementById(id);
    ...
    // Active runs
    const activeRunsListEl = $("active-runs-list");
    const runModal = $("run-modal");
    const runModalTitle = $("run-modal-title");
    const runModalClose = $("run-modal-close");
    const runModalPre = $("run-modal-pre");
    let activeRunModalId = null;

    async function refreshRunOutput(id) {
      if (!runModalPre) return;
      try {
        const res = await fetch("/api/runs/output?id=" + encodeURIComponent(id));
        if (!res.ok) {
          runModalPre.textContent += "\n[run completed]";
          activeRunModalId = null;
          return;
        }
        const data = await res.json();
        runModalPre.textContent = data.output || "(no output yet)";
        runModalPre.scrollTop = runModalPre.scrollHeight;
      } catch {}
    }

    function openRunModal(id, name) {
      activeRunModalId = id;
      if (runModalTitle) runModalTitle.textContent = name;
      if (runModalPre) runModalPre.textContent = "Loading...";
      if (runModal) {
    ...
    function renderActiveRuns(runs) {
      if (!activeRunsListEl) return;
      if (!Array.isArray(runs) || !runs.length) {
        activeRunsListEl.innerHTML = '<div class="active-runs-empty">No active runs.</div>';
        return;
      }
      activeRunsListEl.innerHTML = runs.map((r) =>
        '<button class="active-run-item" type="button" data-run-id="' + escAttr(String(r.id || "")) + '">' +
          '<span class="active-run-dot"></span>' +
          '<span class="active-run-name">' + esc(r.name || "run") + '</span>' +
          '<span class="active-run-elapsed">' + esc(fmtDur(r.elapsedMs)) + '</span>' +
          '<span class="active-run-peek">View →</span>' +
        '</button>'
      ).join("");
    }
    ...
    async function refreshActiveRuns() {
      try {
        ...
        if (activeRunModalId) await refreshRunOutput(activeRunModalId);
      } catch {}
    }

    if (runModalClose && runModal) {
      runModalClose.addEventListener("click", () => {
        activeRunModalId = null;
    ...
    document.addEventListener("click", (event) => {
      if (!runModal) return;
      if (!runModal.classList.contains("open")) return;
    ...
    document.addEventListener("click", (event) => {
      const target = event.target;
      if (!(target instanceof HTMLElement)) return;
      const btn = target.closest("[data-run-id]");
      if (!btn || !(btn instanceof HTMLElement)) return;
      const id = btn.getAttribute("data-run-id") || "";
      const nameEl = btn.querySelector(".active-run-name");
      const name = nameEl ? (nameEl.textContent || id) : id;
      if (id) openRunModal(id, name);
    });
    ...
    // --- Logs pane ---
    const logsDaemonEl = $("logs-daemon");
    const logsRunsEl = $("logs-runs");
    const logsBodyEl = $("logs-body");
    const logsToggleBtn = $("logs-toggle");
    const logsRefreshBtn = $("logs-refresh");
    let logsVisible = localStorage.getItem("logs.visible") !== "0";

    function applyLogsVisibility() {
      if (!logsBodyEl || !logsToggleBtn) return;
      logsBodyEl.classList.toggle("hidden", !logsVisible);
    ...
    if (logsToggleBtn) {
      logsToggleBtn.addEventListener("click", () => {
        ...
        localStorage.setItem("logs.visible", logsVisible ? "1" : "0");
        applyLogsVisibility();
      });
    }
    ...
    async function refreshLogs() {
      try {
        ...
        const data = await res.json();
        if (logsDaemonEl) {
          const lines = Array.isArray(data.daemonLog) ? data.daemonLog : [];
          logsDaemonEl.textContent = lines.length ? lines.join("\n") : "(empty)";
          logsDaemonEl.scrollTop = logsDaemonEl.scrollHeight;
        }
        if (logsRunsEl) {
          const runs = Array.isArray(data.runs) ? data.runs : [];
          if (!runs.length) {
            logsRunsEl.innerHTML = "";
            return;
          }
          logsRunsEl.innerHTML = runs.map((r) =>
            '<div class="logs-section">' +
              '<div class="logs-section-title">' + esc(r.file || "run") + "</div>" +
              '<pre class="logs-pre">' + esc((Array.isArray(r.lines) ? r.lines : []).join("\n") || "(empty)") + "</pre>" +
            "</div>"
          ).join("");
        }
      } catch {
        if (logsDaemonEl) logsDaemonEl.textContent = "(failed to load logs)";
      }
    }
    ...
    setInterval(refreshActiveRuns, 1000);
